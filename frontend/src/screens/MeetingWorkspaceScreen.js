...
function transcriptionKey(transcription, index) {
  return String(
    valueOf(transcription, "id", "Id") ??
      valueOf(transcription, "createdAt", "CreatedAt") ??
      index,
  );
}
...
export default function MeetingWorkspaceScreen({ route }) {
  ...
  const reservationId =
    route.params?.reservationId ?? reservationIdOf(initialReservation);
  ...
  const [expandedTranscriptionKey, setExpandedTranscriptionKey] =
    useState(null);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        "Erreur",
        error?.message || "Impossible de charger la réunion.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleQrScanned = async ({ data }) => {
    ...
    try {
      ...
      showFeedback({
        ...
        message:
          "La réunion est maintenant en cours. Vous pouvez lancer l’enregistrement.",
        confirmText: "Commencer",
      });
    } catch (error) {
      Alert.alert(
        "Erreur",
        error?.message || "Impossible de démarrer la réunion.",
      );
    } finally {
      scanLock.current = false;
      setScanning(false);
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      timerIntervalRef.current = setInterval(() => {
        ...
        setRecordingElapsed(Math.floor(elapsed / 1000));
      }, 1000);
      setIsPaused(false);
  ...
  const pauseRecording = async () => {
    ...
    try {
      ...
      Alert.alert("Erreur", "Impossible de mettre l'enregistrement en pause.");
    }
  };
  ...
  const resumeRecording = async () => {
    ...
    try {
      ...
      Alert.alert("Erreur", "Impossible de reprendre l'enregistrement.");
    }
  };
  ...
  const stopRecordingAndUpload = async () => {
    ...
    try {
      ...
      showFeedback({
        ...
        message:
          "Whisper transcrit l’audio puis l’IA génère le résumé. Cela peut prendre quelques instants.",
      });
      ...
      showFeedback({
        ...
        message:
          "La transcription, le résumé et les tâches ont été générés avec succès.",
      ...
      Alert.alert(
        ...
        error?.message || "Impossible d'envoyer l'enregistrement.",
      );
    } finally {
    if (timerIntervalRef.current) {
        clearInterval(timerIntervalRef.current);
  ...
  const finishMeeting = () => {
    Alert.alert(
      "Terminer la réunion ?",
      "La salle sera libérée immédiatement.",
      [
          ...
          onPress: async () => {
            ...
            try {
              ...
              showFeedback({
                ...
                message:
                  "La salle a été libérée et les données de réunion sont sauvegardées.",
                confirmText: "OK",
              });
            } catch (error) {
              Alert.alert(
                "Erreur",
                error?.message || "Impossible de terminer la réunion.",
              );
            } finally {
              setFinishing(false);
            }
        ...
        <View style={styles.hero}>
          <View style={styles.titleRow}>
            ...
            <View style={styles.titleCopy}>
              ...
              <Text style={styles.dateText}>
                {formatDateTime(
                  valueOf(reservation, "startDateTime", "StartDateTime"),
                )}
              </Text>
            </View>
            <View style={[styles.badge, { borderColor: statusConfig[1] }]}>
              <Text style={[styles.badgeText, { color: statusConfig[1] }]}>
                {statusConfig[0]}
              </Text>
            </View>
          </View>
          <InfoRow
            ...
            value={formatDateTime(
              valueOf(reservation, "endDateTime", "EndDateTime"),
            )}
            styles={styles}
            colors={colors}
          />
          <InfoRow
            ...
            label="Objet"
            value={valueOf(reservation, "purpose", "Purpose") || "Aucun objet"}
            styles={styles}
            colors={colors}
          />
          {!!valueOf(reservation, "startedAt", "StartedAt") && (
            <InfoRow
              icon="play-outline"
              label="Démarrée"
              value={formatDateTime(
          ...
          {!!valueOf(reservation, "finishedAt", "FinishedAt") && (
            <InfoRow
        ...
        {currentStatus === "active" &&
          !valueOf(reservation, "startedAt", "StartedAt") && (
            <ActionButton
              ...
              label="Scanner le QR pour démarrer"
              onPress={openScanner}
              styles={styles}
            />
          )}
          ...
          <View style={styles.card}>
            ...
            {recording ? (
              ...
              <Text style={styles.emptyText}>
                Enregistrez la discussion pour générer un compte rendu.
              </Text>
            )}
            {recording ? (
              <>
            ...
            <TouchableOpacity
              ...
              disabled={finishing || uploading || !!recording}
            ...
            transcriptions.map((item, index) => {
              ...
              const createdTime =
                createdAt && !Number.isNaN(parsedCreatedAt.getTime())
                  ? parsedCreatedAt.toLocaleTimeString("fr-FR", {
                      hour: "2-digit",
                      minute: "2-digit",
                    })
                  : "heure inconnue";
              return (
                <View key={itemKey} style={styles.transcriptionCard}>
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      setExpandedTranscriptionKey(expanded ? null : itemKey)
                    }
                    accessibilityRole="button"
                    accessibilityState={{ expanded }}
                  >
                    <Text style={styles.transcriptionHeaderText}>
                      Enregistrement {transcriptions.length - index} —{" "}
                      {createdTime}
                    </Text>
                    <Ionicons
                  ...
                  {expanded && (
                    <View style={styles.transcriptionBody}>
                      ...
                      <TranscriptSection
                        title="Résumé / points importants"
                        text={valueOf(item, "summary", "Summary")}
                        fallback="Résumé indisponible."
                        styles={styles}
                      />
                      <TranscriptSection
                        title="Tâches"
                        text={valueOf(item, "tasks", "Tasks")}
                        fallback="Aucune tâche détectée."
                        styles={styles}
                      />
                      <TranscriptSection
                        title="Transcription"
                        text={valueOf(item, "transcriptText", "TranscriptText")}
                        fallback="Transcription vide."
                        styles={styles}
                      />
                    </View>
                  )}
  ...
  return (
    <View style={styles.transcriptSection}>
      ...
      <Text style={styles.transcriptText}>
        {cleanMeetingText(text, fallback)}
      </Text>
    </View>
  );
}
